...
//! inválidos, cobrindo pelo menos 10 pares chave/assinatura (spec, Seção 4).
//!
//! A rejeição de casos inválidos acontece em `verify` (exit_code não-SUCCESS
//! do guest é rejeitado por padrão), não em `prove` — ver nota de API em
//! host/verify-mldsa-host/src/lib.rs.
//!
//! Proving em modo compressed é lento (dezenas de segundos por caso, ver
//! docs/decisions.md D1) — rodar com:
...
fn m1_aceita_assinaturas_validas_e_rejeita_invalidas() {
    ...
    for case in cases {
        let outcome = prove_case(&client, &proving_key, &case);
